...
/** @brief   Sets an external time source 
 *  @details if this function is set then the
 *  Unix epoch time is assumed to be generated by this functions <br>
 *  The system Epoch time will then be ignored
 *  @param  funct[in] function pointer to the finction that will generate
 *  the epoch time to be used when generate the tiem stamp. The function
 *  must return the number of seconds sinc 1.Jan 1970 */
void
GTime::SetExternalTimeSource(  std::function<double()>  funct )
{
       fExternalTimeSource = funct; 
}
...
GTime::TimeStamp(struct std::tm *tout, const char *format, struct std::tm *tin, int64_t *us)
{
...

    {
       ...
       if( fExternalTimeSource != nullptr )
       {
